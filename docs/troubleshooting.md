...
The filesystem backend rejects any storage path whose normalised form contains a `pub`, `public`, `public_html`, `web`, `html`, `htdocs`, `wwwroot`, or `httpdocs` segment (case-insensitive) — see `FilesystemStorage::PUBLIC_ROOT_SEGMENTS`. This is a defence-in-depth check: deploy task records embed timestamps and error messages, and writing them under a publicly served document root would expose them over HTTP. The check is scoped to the project directory — only the storage path portion at or below `kernel.project_dir` is inspected, so a server deploy layout like `/var/www/html/` above the project root doesn't trigger a false refusal — and both the storage path and project directory are canonicalized with symlinks resolved at construction time, so a path that only reaches a docroot through a symlink is refused too. Move `storage.filesystem.path` outside the web-served directory — `%kernel.project_dir%/var/deploy-tasks` is the right place. See [`docs/security.md` → Filesystem Storage](security.md#filesystem-storage) for the full guard model and its limitations.

## Group name validation — `\InvalidArgumentException` on `#[AsDeployTask(groups: ...)]`

Group names must match `AsDeployTask::GROUP_NAME_PATTERN` (`^[a-zA-Z0-9._-]+\z`). Slashes, whitespace, accented characters, etc. are rejected. The constraint exists because group names are used verbatim as filename suffixes (`<id>@<group>.json`) and as DBAL primary-key column values; allowing arbitrary input there would invite filesystem path injection and broken queries. Pick a name in the allowed alphabet (`predeploy`, `post-deploy`, `db.warm`, …).
