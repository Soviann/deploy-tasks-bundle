...
final class RegisterTasksCompilerPassTest extends TestCase
{
    ...
    public function testResolveGeneratorClassReturnsDefaultWhenDefinitionHasNullClass(): void
    {
        ...
        // Override deploy_tasks.id_generator with a class-less definition.
        $container->setDefinition('deploy_tasks.id_generator', new Definition());
    ...
    public function testAllOrNothingParameterIsRemovedAfterCompilation(): void
    {
        ...
        $container = $this->baseContainer();
        $container->setParameter('deploy_tasks.runner.all_or_nothing', false);
        ...
        $container->setDefinition('deploy_tasks.storage', $storageDef);

        $pass = new RegisterTasksCompilerPass();
        $pass->process($container);

        self::assertFalse($container->hasParameter('deploy_tasks.runner.all_or_nothing'));
    }

    public function testCustomStorageServiceIdParameterIsRemovedAfterCompilation(): void
    {
        // Mutant 103: remove('deploy_tasks.storage.custom_service_id') removal.
        $container = $this->baseContainer();
        $container->setParameter('deploy_tasks.storage.custom_service_id', 'my.custom_storage');
        ...
        self::assertFalse($container->hasParameter('deploy_tasks.storage.custom_service_id'));
    }

    public function testEventsEnabledParameterIsRemovedAfterCompilation(): void
    {
        // Mutant 127: remove('deploy_tasks.events.enabled') removal.
        $container = $this->baseContainer();
        // events.enabled is already set in baseContainer; run the pass and check.
        $pass = new RegisterTasksCompilerPass();
        $pass->process($container);

        self::assertFalse($container->hasParameter('deploy_tasks.events.enabled'));
    }

    public function testLockEnabledParameterIsRemovedAfterCompilation(): void
    {
        // Mutant 128: remove('deploy_tasks.lock.enabled') removal.
        $container = $this->baseContainer();
        $pass = new RegisterTasksCompilerPass();
        $pass->process($container);

        self::assertFalse($container->hasParameter('deploy_tasks.lock.enabled'));
    }
    ...
    {
        // Mutant 115: `!$container->hasDefinition('deploy_tasks.runner')` flipped to
        ...
        // No 'deploy_tasks.runner' definition.
        ...
        self::assertTrue($container->hasParameter('deploy_tasks.events.enabled'), 'Params remain untouched when runner is absent.');
    }
    ...
    public function testEventDispatcherIsNotWiredWhenEventsDisabled(): void
    {
        ...
        $dispatcherArg = $container->getDefinition('deploy_tasks.runner')->getArgument('$dispatcher');
    ...
    public function testLockFactoryIsWiredWhenLockEnabledAndFactoryPresent(): void
    {
        ...
        $container = $this->baseContainer();
        $container->setParameter('deploy_tasks.lock.enabled', true);
    ...
    public function testLockFactoryIsNotWiredWhenLockDisabled(): void
    {
        ...
        $lockArg = $container->getDefinition('deploy_tasks.runner')->getArgument('$lockFactory');
    ...
    public function testNonTransactionalCustomStorageIsNotAliasedAsTransactional(): void
    {
        ...
        $container = $this->baseContainer();
        $container->setParameter('deploy_tasks.storage.custom_service_id', 'my.storage');

        $storageDef = new Definition(\Soviann\DeployTasksBundle\Storage\InMemory\InMemoryStorage::class);
        $container->setDefinition('my.storage', $storageDef);

        // Also register deploy_tasks.storage alias (needed by validateCustomTransactionalStorage skip).
        $container->setAlias('deploy_tasks.storage', 'my.storage');
    ...
    public function testTransactionalCustomStorageIsAliasedAsTransactionalInterface(): void
    {
        ...
        $container = $this->baseContainer();
        $container->setParameter('deploy_tasks.storage.custom_service_id', 'my.transactional_storage');
        ...
        $container->setAlias('deploy_tasks.storage', 'my.transactional_storage');

        // runner $transactional must be false so validateCustomTransactionalStorage doesn't throw.
        $container->getDefinition('deploy_tasks.runner')->setArgument('$transactional', false);
    ...
    private function baseContainer(): ContainerBuilder
    {
        ...
        $container->getDefinition('deploy_tasks.runner')->setArgument('$dispatcher', null);
        $container->getDefinition('deploy_tasks.runner')->setArgument('$lockFactory', null);
        $container->getDefinition('deploy_tasks.runner')->setArgument('$transactional', false);
        $container->setParameter('deploy_tasks.events.enabled', false);
        $container->setParameter('deploy_tasks.lock.enabled', false);
