...
`deploytasks:run`, `:status`, `:show`, `:skip`, `:reset`, `:rollup`, `:generate:container`, `:host:generate`, `:create-schema` (only for storages implementing `SchemaManageableInterface`, e.g. database storage).
